...
function CareForm() {
  ...
  return (
    <Card className="p-4 shadow-sm">
      <h2 className="text-center mb-4">Criar Cuidado</h2>
      ...
      <Formik
        ...
        onSubmit={async (values, { setSubmitting }) => {
          try {
            await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}/api/cares`, values);
            setSuccess('Cuidado criado com sucesso!');
            setError('');
            setTimeout(() => navigate('/cares'), 2000);
          } catch (err) {
            setError(err.response?.data?.error || 'Erro ao criar cuidado');
            setSuccess('');
          }
          setSubmitting(false);
        }}
                    ...
                    <div key={animal.id} className="mb-2 p-3 border rounded">
                      <BootstrapForm.Check
                        ...
                        onChange={(e) => {
                          ...
                          if (e.target.checked) {
                            assignments.push({ animalId: animal.id, caregiverId: null, status: 'Em andamento' });
                          } else {
                        ...
                        <div className="mt-2">
                          <BootstrapForm.Group>
                            ...
                            <BootstrapForm.Select
                              onChange={(e) => {
                                const assignments = [...values.animalCareAssignments];
                                const idx = assignments.findIndex(a => a.animalId === animal.id);
                                assignments[idx].caregiverId = e.target.value ? parseInt(e.target.value) : null;
                                setFieldValue('animalCareAssignments', assignments);
                              }}
                            >
                          ...
                          <BootstrapForm.Group className="mt-2">
                            ...
                            <BootstrapForm.Select
                              ...
                              value={values.animalCareAssignments.find(a => a.animalId === animal.id)?.status}
                            >
